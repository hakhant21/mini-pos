<?php

namespace Database\Seeders;

use App\Models\Category;
use Illuminate\Database\Seeder;

class CategorySeeder extends Seeder
{
    public function run(): void
    {
        $categories = [
            [
                "id" => 1,
                "name" => "၂၀၀ တန်",
                "slug" => "200-tan",
                "description" => "၂၀၀ တန်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:44:44",
                "updated_at" => "2026-07-20 15:55:28"
            ],
            [
                "id" => 2,
                "name" => "၃၀၀ တန်",
                "slug" => "300-tan",
                "description" => "၃၀၀ တန်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:44:55",
                "updated_at" => "2026-07-20 15:55:43"
            ],
            [
                "id" => 3,
                "name" => "၆၀၀ တန်",
                "slug" => "600-tan",
                "description" => "၆၀၀ တန်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:45:12",
                "updated_at" => "2026-07-20 15:55:53"
            ],
            [
                "id" => 4,
                "name" => "၇၀၀ တန်",
                "slug" => "700-tan",
                "description" => "၇၀၀ တန်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:45:12",
                "updated_at" => "2026-07-20 15:55:53"
            ],
            [
                "id" => 5,
                "name" => "၈၀၀ တန်",
                "slug" => "800-tan",
                "description" => "၈၀၀ တန်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:45:12",
                "updated_at" => "2026-07-20 15:55:53"
            ],
            [
                "id" => 6,
                "name" => "၁၀၀၀ တန်",
                "slug" => "1000-tan",
                "description" => "၁၀၀၀ တန်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:48:34",
                "updated_at" => "2026-07-20 15:56:02"
            ],
            [
                "id" => 7,
                "name" => "၁၂၀၀ တန်",
                "slug" => "1200-tan",
                "description" => "၁၂၀၀ တန်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:51:07",
                "updated_at" => "2026-07-20 15:51:07"
            ],
            [
                "id" => 8,
                "name" => "၁၅၀၀ တန်",
                "slug" => "1500-tan",
                "description" => "၁၅၀၀ တန်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:51:20",
                "updated_at" => "2026-07-20 15:56:12"
            ],
            [
                "id" => 9,
                "name" => "ဖက်ကြမ်း",
                "slug" => "paetkyan",
                "description" => "ဖက်ကြမ်း",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:52:21",
                "updated_at" => "2026-07-20 15:52:21"
            ],
            [
                "id" => 10,
                "name" => "စီးကရက်",
                "slug" => "cigarettes",
                "description" => "စီးကရက်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:53:57",
                "updated_at" => "2026-07-20 15:55:14"
            ],
            [
                "id" => 11,
                "name" => "ဘီယာ",
                "slug" => "baiya",
                "description" => "ဘီယာ",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:54:31",
                "updated_at" => "2026-07-20 15:54:31"
            ],
            [
                "id" => 12,
                "name" => "အရက်",
                "slug" => "ayaet",
                "description" => "အရက်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:54:35",
                "updated_at" => "2026-07-20 15:54:42"
            ],
            [
                "id" => 13,
                "name" => "အချိုရည်",
                "slug" => "akhyaoyai",
                "description" => "အချိုရည်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 15:59:06",
                "updated_at" => "2026-07-20 15:59:06"
            ],
            [
                "id" => 14,
                "name" => "ဆိုလ်ဂျူး",
                "slug" => "saolgyau",
                "description" => "ဆိုလ်ဂျူး",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 16:03:33",
                "updated_at" => "2026-07-20 16:03:33"
            ],
            [
                "id" => 15,
                "name" => "ဝိုင်",
                "slug" => "waing",
                "description" => "ဝိုင်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 16:03:46",
                "updated_at" => "2026-07-20 16:03:46"
            ],
            [
                "id" => 16,
                "name" => "ရေသန့်",
                "slug" => "yaethan",
                "description" => "ရေသန့်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 16:06:11",
                "updated_at" => "2026-07-20 16:06:11"
            ],
            [
                "id" => 17,
                "name" => "ခေါက်ဆွဲခြောက်",
                "slug" => "khawetsawekhyauk",
                "description" => "ခေါက်ဆွဲခြောက်",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 16:07:25",
                "updated_at" => "2026-07-20 16:07:25"
            ],
            [
                "id" => 18,
                "name" => "အခြား",
                "slug" => "acharr",
                "description" => "အခြား",
                "image" => null,
                "is_active" => true,
                "deleted_at" => null,
                "created_at" => "2026-07-20 16:07:25",
                "updated_at" => "2026-07-20 16:07:25"
            ]
        ];

        foreach ($categories as $category) {
            Category::create($category);
        }
    }
}
