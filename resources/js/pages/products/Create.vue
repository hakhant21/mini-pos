...
type UnitForm = { name: string; conversion: number; purchase_price: number; selling_price: number; package_price: number; single_unit_price: number; package_quantity: number; loose_quantity: number; barcode: string };
const form = useForm({ category_id: props.categories[0]?.id ?? '', name: '', sku: '', price_mode: 'standard', base_unit: 'piece', reorder_level: 0, image: null as File | null, units: [{ name: 'Single', conversion: 1, purchase_price: 0, selling_price: 0, package_price: 0, single_unit_price: 0, package_quantity: 0, loose_quantity: 0, barcode: '' }] as UnitForm[] });
                    ...
                    <div class="space-y-5">
                         <div v-for="(unit, index) in form.units" :key="index" class="grid gap-5 rounded-xl border border-slate-100 p-4 dark:border-slate-800 sm:grid-cols-3">
                          ...
                          <FormField :label="$t('products.conversion')" :error="form.errors[`units.${index}.conversion` ]" :hint="$t('products.conversion_hint')" required><input v-model.number="unit.conversion" :class="inputClass" type="number" min="1" required /></FormField>
                             <FormField :label="$t('products.purchase_price')" :error="form.errors[`units.${index}.purchase_price` ]" required><input v-model.number="unit.purchase_price" :class="inputClass" type="number" min="0" required /></FormField>
                                <FormField v-if="form.price_mode !== 'standard'" :label="$t('products.selling_price')" :error="form.errors[`units.${index}.selling_price` ]" required><input v-model.number="unit.selling_price" :class="inputClass" type="number" min="0" required /></FormField>
                                <FormField v-if="form.price_mode === 'single_package_carton'" :label="$t('products.package_price')" :error="form.errors[`units.${index}.package_price` ]" required><input v-model.number="unit.package_price" :class="inputClass" type="number" min="0" required /></FormField>
                                <FormField v-if="form.price_mode !== 'standard'" :label="$t('products.single_unit_price')" :error="form.errors[`units.${index}.single_unit_price` ]" required><input v-model.number="unit.single_unit_price" :class="inputClass" type="number" min="0" required /></FormField>
                             <FormField :label="$t('products.packages')" :error="form.errors[`units.${index}.package_quantity` ]" required><input v-model.number="unit.package_quantity" :class="inputClass" type="number" min="0" required /></FormField>
                             <FormField :label="$t('products.loose_units', { unit: form.base_unit })" :error="form.errors[`units.${index}.loose_quantity` ]" required><input v-model.number="unit.loose_quantity" :class="inputClass" type="number" min="0" required /></FormField>
                      </div>
