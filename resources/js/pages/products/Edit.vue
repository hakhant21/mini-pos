...
const form = useForm({ category_id: props.product.category_id, name: props.product.name, sku: props.product.sku, price_mode: props.product.price_mode, base_unit: props.product.base_unit, reorder_level: props.product.reorder_level, active: props.product.active, image: null as File | null, units: props.product.units });
...
function addUnit(): void {
    form.units.push({ name: '', conversion: 1, purchase_price: 0, selling_price: 0, package_price: 0, single_unit_price: 0, package_quantity: 0, loose_quantity: 0, barcode: '' });
}
        ...
        <div class="mx-auto max-w-7xl space-y-6">
            ...
            <form @submit.prevent="submit" class="space-y-5">
                        ...
                        <FormField :label="$t('products.product_name')" :error="form.errors.name" required><input v-model="form.name" :class="inputClass" required /></FormField>
                         <FormField :label="$t('products.sku')" :error="form.errors.sku" :hint="$t('products.sku_edit_hint')" required><input v-model="form.sku" :class="inputClass" pattern="[A-Z0-9-]+" required /></FormField>
                          <FormField :label="$t('products.category')" :error="form.errors.category_id" required><select v-model="form.category_id" :class="inputClass" required><option v-for="category in props.categories" :key="category.id" :value="category.id">{{ category.name }}</option></select></FormField>
                           <FormField label="Price mode" :error="form.errors.price_mode" required><select v-model="form.price_mode" :class="inputClass" required><option value="standard">Single only</option><option value="single_package">Single and Package</option><option value="single_package_carton">Single, Package and Carton</option></select></FormField>
                ...
                <FormSection :title="$t('products.selling_units')" :description="$t('products.selling_units_edit_description')">
                     <div class="space-y-5">
                         <p class="text-sm text-slate-500">{{ $t('products.purchase_price') }}, {{ $t('products.selling_price') }}, {{ $t('products.packages') }} and {{ $t('products.loose_units', { unit: form.base_unit }) }} are set per unit below.</p>
                       <div v-for="(unit, index) in form.units" :key="unit.id ?? index" class="grid gap-5 rounded-xl border border-slate-100 p-4 dark:border-slate-800 sm:grid-cols-3">
                            <FormField v-if="form.price_mode === 'single_package_carton'" class="order-5" :label="$t('products.package_price')" :error="errors[`units.${index}.package_price`]" required><input v-model.number="unit.package_price" :class="inputClass" type="number" min="0" required /></FormField>
                            <FormField v-if="form.price_mode !== 'standard'" class="order-6" :label="$t('products.single_unit_price')"><input v-model.number="unit.single_unit_price" :class="inputClass" type="number" min="0" required /></FormField>
                         <FormField class="order-1" :label="$t('products.unit_name')" :error="errors[`units.${index}.name`]"><input v-model="unit.name" :class="inputClass" required /></FormField>
                          <FormField class="order-2" :label="$t('products.conversion')" :error="errors[`units.${index}.conversion`]"><input v-model.number="unit.conversion" :class="inputClass" type="number" min="1" required /></FormField>
                          <FormField v-if="form.price_mode !== 'standard'" class="order-4" :label="$t('products.selling_price')" :error="errors[`units.${index}.selling_price`]"><input v-model.number="unit.selling_price" :class="inputClass" type="number" min="0" required /></FormField>
                          <FormField class="order-3" :label="$t('products.purchase_price')" :error="errors[`units.${index}.purchase_price`]"><input v-model.number="unit.purchase_price" :class="inputClass" type="number" min="0" required /></FormField>
                          <FormField class="order-7" :label="$t('products.packages')" :error="errors[`units.${index}.package_quantity`]"><input v-model.number="unit.package_quantity" :class="inputClass" type="number" min="0" required /></FormField>
                          <FormField class="order-8" :label="$t('products.loose_units', { unit: form.base_unit })" :error="errors[`units.${index}.loose_quantity`]"><input v-model.number="unit.loose_quantity" :class="inputClass" type="number" min="0" required /></FormField>
                      </div>
